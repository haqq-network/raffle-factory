...
describe("RaffleFactory full flow", function () {
  ...
  it("Should return prize to factory and allow admin to withdraw if no participants", async function () {
    const [owner, manager, participant1] = await ethers.getSigners();
    ...
    // Complete the raffle via factory
    await factory.connect(manager).finishRaffle(raffleId);

    // The prize should return to the factory
    expect(await erc20.balanceOf(factory.target)).to.equal(500);
    expect(await erc20.balanceOf(raffleAddress)).to.equal(0);

    // Admin withdraws the prize from the factory
    await factory.connect(owner).withdraw(erc20.target, owner.address, 500);
